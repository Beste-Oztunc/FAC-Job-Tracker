...
RESUME_VERSIONS = {
    "AI / Solutions Engineering": (
        "Use for solutions engineering, technical presales, enterprise AI, "
        "GenAI, agentic AI, API integration, AI governance, and platform roles."
    ),
    "Risk / Fraud / Model Governance": (
        "Use for credit risk, fraud, AML/KYC, model risk, validation, "
        "decisioning, collections, lending, banking, and regulated-industry roles."
    ),
    "Master / Broad Enterprise": (
        "Use for cross-functional enterprise consulting, mixed risk-and-AI roles, "
        "customer success, technical strategy, and roles with broad scope."
    ),
    "Economist / Quantitative": (
        "Use for economist, economic research, forecasting, quantitative analysis, "
        "macroeconomic modeling, and policy-adjacent roles."
    ),
}
...
def deterministic_market_fallback(stats: dict) -> dict:
    cloud = stat_lookup(stats, "Cloud architecture")
    kubernetes = stat_lookup(stats, "Kubernetes")
    genai = stat_lookup(stats, "Generative AI")
    fraud = stat_lookup(stats, "Fraud, AML, and KYC")
    risk = stat_lookup(stats, "Risk and credit")

    cloud_vendor_counts = [
        stat_lookup(stats, "Microsoft Azure"),
        stat_lookup(stats, "AWS"),
        stat_lookup(stats, "Google Cloud"),
    ]
    cloud_vendor_counts.sort(
        key=lambda item: item["count"],
        reverse=True,
    )
    leading_cloud = cloud_vendor_counts[0]

    return {
        "executive_summary": (
            "The local market statistics are ready. Add an OpenAI API key "
            "to generate a personalized narrative and per-job coaching. "
            "The counts and percentages shown on this page are already "
            "calculated directly from the job descriptions."
        ),
        "strongest_alignment": [
            "Risk, fraud, credit, lending, and decisioning",
            "Technical discovery, demonstrations, workshops, and proof-of-concept delivery",
            "Enterprise AI, agentic workflows, and model governance",
            "Regulated-industry communication and executive translation",
        ],
        "gaps": [
            {
                "area": "Cloud deployment architecture",
                "priority": "High" if cloud["count"] else "Medium",
                "why_it_matters": (
                    "Cloud architecture is repeatedly requested in the relevant market."
                ),
                "market_evidence": (
                    f"{cloud['count']} of {stats['relevant_job_count']} relevant jobs "
                    f"mention cloud architecture concepts."
                ),
                "practical_next_step": (
                    "Build and document one end-to-end deployment architecture "
                    "for a regulated AI decisioning workload."
                ),
            },
            {
                "area": "Kubernetes and container concepts",
                "priority": "High" if kubernetes["count"] >= 10 else "Medium",
                "why_it_matters": (
                    "Container orchestration often separates prototype experience "
                    "from production-platform credibility."
                ),
                "market_evidence": (
                    f"Kubernetes appears in {kubernetes['count']} relevant listings."
                ),
                "practical_next_step": (
                    "Learn deployment, services, ingress, autoscaling, secrets, "
                    "observability, and basic troubleshooting."
                ),
            },
            {
                "area": "Production LLM evaluation and monitoring",
                "priority": "High" if genai["count"] >= 10 else "Medium",
                "why_it_matters": (
                    "The profile strongly demonstrates prototyping; production "
                    "monitoring evidence should be equally visible."
                ),
                "market_evidence": (
                    f"Generative AI appears in {genai['count']} relevant listings."
                ),
                "practical_next_step": (
                    "Create a portfolio case covering quality metrics, drift, "
                    "latency, cost, hallucination risk, guardrails, and escalation."
                ),
            },
        ],
        "certification_priorities": [
            {
                ...
                "recommended_options": [
                    (
                        f"A certification in {leading_cloud['name']} architecture"
                        if leading_cloud["count"]
                        else "A major-cloud architecture certification"
                    )
                ],
                "why_this_track": (
                    "It closes the most visible architecture credibility gap "
                    "for technical presales and solutions roles."
                ),
                "job_market_evidence": (
                    f"{leading_cloud['name']} appears in "
                    f"{leading_cloud['count']} relevant listings."
                ),
                "skills_it_would_strengthen": [
                ...
                "caution": (
                    "Verify the current exam path, prerequisites, and pricing "
                    "on the vendor's official site."
                ),
            },
            {
                "rank": 2,
                "track": "Vendor-neutral Kubernetes fundamentals",
                "recommended_options": [
                    "Kubernetes fundamentals or administrator-oriented credential"
                ],
                "why_this_track": (
                    "It strengthens production deployment conversations without "
                    "repositioning the candidate as a software engineer."
                ),
                "job_market_evidence": (
                    f"Kubernetes appears in {kubernetes['count']} relevant listings."
                ),
                ...
                "caution": (
                    "Choose a practical fundamentals path before committing to "
                    "a deeply operations-focused credential."
                ),
            },
            {
                "rank": 3,
                "track": "AI engineering and GenAI deployment",
                "recommended_options": [
                    "AI engineering credential emphasizing deployment and evaluation"
                ],
                "why_this_track": (
                    "It turns strong GenAI prototyping experience into a clearer "
                ...
                "caution": (
                    "Avoid credentials that are mostly introductory prompt-engineering material."
                ),
            },
            {
                "rank": 4,
                "track": "Risk and fraud specialization",
                "recommended_options": [
                    "Recognized fraud credential",
                    "Recognized financial-risk credential",
                ],
                "why_this_track": (
                    "This reinforces an existing differentiator rather than "
                    "creating a new career direction."
                ),
                "job_market_evidence": (
                    f"Risk/credit appears in {risk['count']} relevant listings; "
                    f"fraud/AML/KYC appears in {fraud['count']}."
                ),
                "skills_it_would_strengthen": [
                ...
                "caution": (
                    "Prioritize only when the target role values formal domain credentials."
                ),
            },
        ],
        "market_signals": [
            (
                f"{leading_cloud['name']} is the most frequently mentioned cloud "
                f"platform in this relevant-job slice ({leading_cloud['count']} listings)."
            ),
            (
                "The strongest differentiation remains the combination of "
                "customer-facing technical work, regulated AI, and risk domain depth."
            ),
        ],
        "recommended_30_day_actions": [
            "Create one cloud architecture diagram for a regulated AI decisioning workflow.",
            "Prepare quantified stories covering recovery, conversion, time-to-production, and sales impact.",
            "Build a concise production LLM evaluation and monitoring case study.",
            "Map the highest-ranked cloud credential against the actual listing counts on this page.",
        ],
    }
